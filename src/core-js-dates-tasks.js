...
function getDayName(date) {
  ...
  return tagen.find((_, index) => index === data);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diff = Math.abs(new Date(dateEnd) - new Date(dateStart));
...
function formatDate(date) {
  ...
  return datum.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      result.push(formatDates(currentDate));
